...
def generate(upstream_dir: str, out_dir: str) -> dict:
    ...
    identity["hans_exclusive"] = len({c for c in per_face["hans"] - per_face["hant"] if is_han(c)})
    identity["hant_exclusive"] = len({c for c in per_face["hant"] - per_face["hans"] if is_han(c)})
